...
//  PracticePlaygrounds
//
//  Created by Zhiyuan Zhou on 5/17/23.
//
...
//        drawImage(rect)
  ...
  private func drawGraph(_ rect: CGRect) {
    ...
    let data: [CGFloat] = [10, 10, 22, 28, 23, 19, 19].map { value in
      return value * 5
    }
    let graphWidth: CGFloat = UIScreen.main.bounds.width
    ...
    let segLen = data.count - 1
    for k in 0..<segLen {
      let x = CGFloat(k) * UIScreen.main.bounds.width / CGFloat(segLen)
      let y = data[k]
      ...
      if k == 0 {
        ...
        let smooth = 1.0
        let nextIdx = k + 1
        let nextX = CGFloat(nextIdx) * UIScreen.main.bounds.width / CGFloat(segLen)
        let nextY = data[nextIdx]
        
        let tmpK = k + 1
        var ratioNextSeg = 0.5
        var vx: CGFloat = 0
        var vy: CGFloat = 0
        var nextCpx0: CGFloat = 0
        var nextCpy0: CGFloat = 0
        ...
        if tmpK > segLen {
          ...
          let dy0 = y - prevY
          let dy1 = nextY - y
          let lenPrevSeg = sqrt(dx0 * dx0 + dy0 * dy0)
          let lenNextSeg = sqrt(dx1 * dx1 + dy1 * dy1)
          ...
          nextCpx0 = min(nextCpx0, max(nextX, x))
          nextCpy0 = min(nextCpy0, max(nextY, y))
          nextCpx0 = max(nextCpx0, min(nextX, x))
          nextCpy0 = max(nextCpy0, min(nextY, y))
          ...
          cpx1 = min(cpx1, max(prevX, x))
          cpy1 = min(cpy1, max(prevY, y))
          cpx1 = max(cpx1, min(prevX, x))
          cpy1 = max(cpy1, min(prevY, y))
        ...
        path.addCurve(to: CGPoint(x: x, y: y), control1: CGPoint(x: cpx0, y: cpy0), control2: CGPoint(x: cpx1, y: cpy1))
        cpx0 = nextCpx0
        cpy0 = nextCpy0
      }
      prevX = x
